...
export default function GlobalUserMenu({ className = '' }: GlobalUserMenuProps) {
  ...
  const [isPasswordSameAsStudentId, setIsPasswordSameAsStudentId] = useState(false)
  ...
  // 检查密码是否与学号一致
  const checkPasswordMatchesStudentId = async (): Promise<boolean> => {
    try {
      if (!userInfo?.student_id) return false
      ...
      const response = await fetch('/api/auth', {
        ...
        body: JSON.stringify({
          ...
          password: userInfo.student_id // 用学号作为密码尝试登录
        })
      })

      const result = await response.json()
      return result.success // 如果登录成功，说明密码确实与学号一致
    } catch (error) {
  ...
  const handlePasswordChangeClick = async () => {
    ...
    // 检查密码是否与学号一致
    const passwordMatches = await checkPasswordMatchesStudentId()
    setIsPasswordSameAsStudentId(passwordMatches)
  ...
  const handleChangePassword = async (newPassword: string): Promise<boolean> => {
    try {
      // 获取当前密码（如果密码与学号一致，则使用学号；否则需要用户提供）
      const currentPassword = isPasswordSameAsStudentId ? userInfo?.student_id || '' : userInfo?.student_id || ''
  ...
  return (
    <div className={`relative ${className}`} ref={menuRef}>
      ...
      <PasswordChangeModal
        ...
        currentPassword={isPasswordSameAsStudentId ? userInfo?.student_id || '' : ''}
      />
    </div>
  )
}
