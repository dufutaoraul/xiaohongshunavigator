...
export async function GET(request: NextRequest) {
  try {
    ...
    if (studentId) {
      ...
      // 获取所有学员的打卡记录（用于管理员页面）
      console.log(`📊 [Checkin Records API] 获取所有学员打卡记录, limit: ${limit}, offset: ${offset}`)
      ...
      console.log(`🔍 [Checkin Records API] 所有记录查询结果:`, {
        records: records,
        recordsLength: records?.length || 0,
        error: error
      })
      ...
      if (error) {
        console.error('❌ [Checkin Records API] 查询所有打卡记录失败:', error)
        return NextResponse.json(
          { error: 'Failed to fetch all checkin records: ' + error.message },
          { status: 500 }
        )
      }
      ...
      if (countError) {
        console.error('查询所有打卡记录总数失败:', countError)
      }
